...
Reproducibility manifest schema v1 inventories each expected artifact with its relative path, type, schema/header, size, row count, byte hash where stable, semantic hash, reproducibility level, validator, required status, and parent lineage. Reconstruction reports use report schema v1 and distinguish exact, semantic, presentation-only, environment-only, failed, and not-applicable outcomes. See [Reproducibility](REPRODUCIBILITY.md).
